...
bool HookConfig::Initialize(filename_t filename) {
    if (isOpen) {
...
static ConfigWatch *g_properties;

void ConfigWatch::Register()
{
    this->next = g_properties;
    g_properties = this;
}

void ConfigWatch::ReloadAll()
{
    for (auto func = g_properties; func != nullptr; func = func->next) {
        func->Reload();
    }
}